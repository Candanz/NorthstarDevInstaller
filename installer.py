...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.repos = {k: tk.BooleanVar(value=True) for k in REPOS}
    ...
    def run_install(self):
        ...
        for name, var in p.repos.items():
            if var.get():
                ...
                subprocess.run(f'git clone {repo_info["url"]} "{repo_path}"', shell=True)

        # 3. Patch SQL
        patched_sql = patch_sql(p.sql_file.get(), p.db_name.get(), self.log)

        # 4. Detect MySQL
        mysql_path = detect_xampp_mysql()
        if not mysql_path:
            p.errors.append("MySQL executable not found. Please ensure XAMPP is installed.")
            self.log("[ERROR] MySQL not found")
            return

        # 5. Run SQL
        self.log(f"[INFO] Using MySQL at: {mysql_path}")
        self.log(f"[INFO] Database: {p.db_name.get()}, User: {p.db_user.get()}")
        self.log("[INFO] Running SQL... This may take a moment.")
        run_sql(mysql_path, p.db_name.get(), p.db_user.get(), p.db_pass.get(), patched_sql, self.log, p.errors)
...
class Finish(ctk.CTkFrame):
    def __init__(self,p):
        ...
        if p.errors:
            ctk.CTkLabel(self,text="However, some errors were encountered:", text_color="red").pack(pady=10)
            for err in p.errors:
                ctk.CTkLabel(self,text=err, text_color="red").pack(pady=5)
        else:
            ctk.CTkLabel(self,text="No errors encountered.", text_color="green").pack(pady=10)
